...
<div class="accesories-wrapper">
    <div class="accesories" v-for="accesories in AccesoriesArray" :key="accesories.id" >
        <CardProduct @delete="onDelete(accesories.id)"  :title="accesories.name" :price="accesories.price" :quantity="accesories.quantity" :description="accesories.description" :id="accesories.id" :src = "accesories.imgSrc"/>
    </div> 
</div>
...
export default {
    ...
    methods:{
       ...
       onDelete(id){
            this.AccesoriesArray.splice(id,1)
            for (let index = 0; index < this.AccesoriesArray.length; index++) {
                this.AccesoriesArray[index].id=index
                console.log(this.AccesoriesArray[index])
            }

            localStorage.setItem('accesories',JSON.stringify(this.AccesoriesArray))
       ...
       updateList(){
              if(!isNaN(this.product.price) && !isNaN(this.product.quantity) && this.product.name!='' && this.product.description!='' && this.product.price!='' && this.product.quantity!=''){
                this.AccesoriesArray.push(
                {
                    name: this.product.name,
                    price: this.product.price + ' $',
                    quantity:this.product.quantity,
                    description:this.product.description,
                    id: this.AccesoriesArray.length,
