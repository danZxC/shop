...
<div class="pc-wrapper">
    <div class="pc" v-for="pc in PcArray" :key="pc.id" >
        <CardProduct @delete="onDelete(pc.id)"  :title="pc.name" :price="pc.price" :quantity="pc.quantity" :description="pc.description" :id="pc.id" :src = "pc.imgSrc"/>
    </div> 
</div>
...
export default {
    ...
    methods:{
       ...
       onDelete(id){
            this.PcArray.splice(id,1)
            for (let index = 0; index < this.PcArray.length; index++) {
                this.PcArray[index].id=index
                console.log(this.PcArray[index])
            }
